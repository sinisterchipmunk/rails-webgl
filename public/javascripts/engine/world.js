...
World.prototype = {
  ...
  /*
    Returns the unique Object ID of the object at the specified x, y
    coordinates. The return value is undefined if there is no object
  ...
  */
  ...
  /*
    Returns the object at the specified x, y coordinates, or null if
    there is no registered object at that location.
  */
  pick: function(x, y) {
  ...
  /*
    Returns an array of all objects in the given rectangle. The array
    is empty if there are no objects in the region.
  */
  ...
  /*
    Returns an array of unique Object IDs for each object in the specified
    region. The array is empty if there are no objects in the region.
  */
  pickRectIndices: function(x, y, w, h) {
    // we want to begin reading at the pixel x, y --
    // so we really need to start reading at x-1, y-1 with a minimum width
    ...
    if (!data) {
    */
      data = new Uint8Array(w*h*4); // w * h * 4, where 4 represents RGBA
